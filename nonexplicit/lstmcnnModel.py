...
class train_model():
    ...
    def fit(self, model, filename):
        ...
        dev = self.dev_data
        test = self.test_data
        stop = EarlyStopping(patience=8, verbose=1,monitor='val_acc',mode='max')
...
if __name__ == '__main__':
    ...
    dev_data = p.data_prepare(p.test_file, False)
    test_data = p.data_prepare(p.dev_file, False)
    WE = p.w2v_process()
    # ch_flength = [[1,2,3]]
    # word_flength = [[2,4,8]]
    # cn_fnb = (128,256)
    ...
    for i in range(5):
        for dsize in (100,500,800):
            try:
                ...
                model.fit(model.lstmCh_word_pro(),"lstmcnn_final")
            except Exception as e:
                print (Exception,":",e)
