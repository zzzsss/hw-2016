...
class precessing():
    def __init__(self):
        self.file = "pdtb_data/"
        self.train_file = self.file + "train_pdtb.json"
        self.dev_file = self.file + "dev_pdtb.json"
        self.test_file = self.file + "test_pdtb.json"
    ...
    def data_prepare(self, file, iftrain):
        ...
        for x in data:
            if iftrain:
                ...
                arg1.append(x["Arg1"])
                arg2.append(x["Arg2"])
                sense.append(Sense_To_Label[x['Sense'][0]])
                allx.append(x)
    ...
    def w2v_process(self):
        ...
        WE = np.zeros((33000,self.w2v_dims),dtype='float32')
...
class train_model():
    ...
    def lstmCh_word_pro(self):
        ...
        arg1_lstm_both = merge([arg1_lstmf,arg1_lstmb,arg1_word],mode='concat')
        arg2_lstm_both = merge([arg2_lstmf,arg2_lstmb,arg2_word],mode='concat')

        arg1_lstm = Dropout(self.dropout)(arg1_lstm_both)
        arg2_lstm = Dropout(self.dropout)(arg2_lstm_both)
        ...
        #another without training
        model2 = Model(input=[arg1_ch_input, arg2_ch_input, arg1_word_input, arg2_word_input],
                      output=[arg1_ch_cnn_merge, arg2_ch_cnn_merge, arg1_lstm_both, arg2_lstm_both, arg1_word, arg2_word])
    ...
    def fit(self, model, filename):
        ...
        ''' special test '''
        pred_pr_return = model.predict({'arg1_ch': self.reshape_arg(dev[0]), 'arg2_ch': self.reshape_arg(dev[1]),
                                 'arg1_word': dev[2], 'arg2_word': dev[3]})
        pred_pr = pred_pr_return
        pred_y = [np.argmax(z) for z in pred_pr]
        print("Real test acc: ", train_model.get_acc(pred_y, dev[5]))

        # ''' write record '''
        # try:
        #     csvfile = open('final.csv', 'a+')
        #     writer = csv.writer(csvfile, dialect='excel')
        #     writer.writerow([filename, train_model.get_acc(pred_y, dev[5]),val_acc, test_acc[1],self.lr,self.dense_num,self.lstm_size,self.ch_filter_length[0],self.ch_filter_length[1],self.ch_filter_length[2],
    ...
    def get_acc(pred_y, allx):
        ...
        for pi, g in zip(pred_y, allx):
            num_all += 1
            if Label_To_Sense[pi] in g["Sense"]:
                num_right += 1
        return (num_right+0.) / num_all

def TMP_reshape_arg(arg):
    X = np.reshape(arg, (arg.shape[0], arg.shape[1] * arg.shape[2]))
    return X

if __name__ == '__main__':
    p = precessing()
    p.data_prepare("pdtb_data/train_vocab.json", True)
    train_data = p.data_prepare(p.train_file, False)
    ...
    # cn_fnb = (128,256)
    # word_fnb = (256,512)
    cn_nb_filter1,word_nb_filter,ch_filter_length,word_filter_length = 128,1024,(2,3,4),(2,4,8)
    # for i in range(5):
    #     for dsize in (100,500,800):
    if True:
        if True:
            i = 1
            dsize = 100
            if True:
                model = train_model(train_data, dev_data, test_data, WE,
                                    ch_filter_length=ch_filter_length, ch_nb_filter=128,
                                    word_filter_length=word_filter_length, word_nb_filter=word_nb_filter,
                                    lstm_size=50,dense_num=i,dense_size=dsize,dropout=0.25,batch_size=128,lr=0.002)
                m1, m2 = model.lstmCh_word_pro()
                model.fit(m1, "lstmcnn_final_extract")
                # get the embeddings of dev -> a1-cnn, a2-cnn, a1-enhance, a2-enhance, a1-w, a2-w
                output_embeddings = m2.predict({'arg1_ch': TMP_reshape_arg(train_data[0]), 'arg2_ch': TMP_reshape_arg(train_data[1]),
                                 'arg1_word': train_data[2], 'arg2_word': train_data[3]})
                for one in output_embeddings:
                    print(one.shape)
                #store them
                dev_words = p.arg_process(p.train_file)
                all_them = []   # [sent_index, arg_index, word_index, word, sentence_words, [cnn], [enhanced], [w]]
                MAX_LENGTH = 80
                assert len(dev_words) == len(output_embeddings[0])
                for j, sent in enumerate(dev_words):
                    # arg1
                    x = sent["Arg1"]
                    for i, w in enumerate(x):
                        if i >= len(x)-MAX_LENGTH:
                            ind = i-len(x)   # minus index
                            all_them.append([j, 1, i, w, x, output_embeddings[0][j][ind].tolist(),
                                             output_embeddings[2][j][ind].tolist(), output_embeddings[4][j][ind].tolist()])
                    # arg2
                    x = sent["Arg2"]
                    for i, w in enumerate(x):
                        if i < MAX_LENGTH:
                            ind = i
                            all_them.append([j, 2, i, w, x, output_embeddings[1][j][ind].tolist(),
                                             output_embeddings[3][j][ind].tolist(), output_embeddings[5][j][ind].tolist()])
                print("Writing output ones.")
                with open("output_embeddings-train.json", "w") as f:
                    for one in all_them:
